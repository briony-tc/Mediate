...
# into /dev/sr0. Rips with the MakeMKV CLI (no GUI dialogs), reports live
# progress to media-library-shelf while ripping, then tells it the rip is
# done via /api/rip-complete.
#
...
# "all" rips every title MakeMKV's own minlength setting would already have
# pre-checked in the GUI - this mirrors current manual behavior, not a new
# heuristic, since it reads the same settings.conf the GUI does.
#
